...
func TestStreamingPreludeKeepAliveEnabledRequiresSwitchAndInterval(t *testing.T) {
	tests := []struct {
		...
		{name: "disabled with passthrough headers", cfg: &sdkconfig.SDKConfig{PassthroughHeaders: true, Streaming: sdkconfig.StreamingConfig{KeepAliveSeconds: 1, PreludeKeepAlive: true}}, want: false},
	}

...
func TestUpstreamPump_ClaudePreservesMaxTokensStopReason(t *testing.T) {
	...
	if !meta.StreamCompleted {
		t.Fatal("message_stop must mark the upstream stream complete")
	}
}

func TestUpstreamPump_ClaudeTruncatedStreamDoesNotMarkComplete(t *testing.T) {
	sse := "data: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_partial\",\"model\":\"m\"}}\n\ndata: {\"type\":\"content_block_delta\",\"index\":0,\"delta\":{\"type\":\"text_delta\",\"text\":\"partial\"}}\n\n"
	spy := &spyEvents{}
	parser := NewStreamParser(spy.events(), UpstreamMeta{}, DefaultFenceToken)
	pump := &UpstreamPump{Reader: strings.NewReader(sse), Shape: ShapeClaudeMessages, Parser: parser}
	meta, err := pump.Run(context.Background())
	...
	if meta.StreamCompleted {
		t.Fatal("EOF without message_stop must not mark the upstream stream complete")
	}
}

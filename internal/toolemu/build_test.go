...
func TestFakeStreamResponses_PreservesIncompleteStatus(t *testing.T) {
	chunks, err := FakeStreamResponses(Parsed{Prose: "partial"}, UpstreamMeta{
		ResponseID:        "resp_1",
		Model:             "m",
		ResponseStatus:    "incomplete",
		IncompleteDetails: []byte(`{"reason":"max_output_tokens"}`),
	...
	if strings.Contains(joined, "event: response.completed") {
		t.Fatalf("incomplete fake stream must not emit completed terminal frame:\n%s", joined)
	}
	if !strings.Contains(joined, `"reason":"max_output_tokens"`) {
		t.Fatalf("incomplete details missing from fake stream:\n%s", joined)
	}
}

...
func (p *UpstreamPump) Run(ctx context.Context) (UpstreamMeta, error) {
	...
	dispatch := func() (bool, error) {
		...
		if errParser := p.Parser.Err(); errParser != nil {
			return false, p.wrapParserError(errParser)
		}
		return done, nil
	}
	...
	if len(dataLines) > 0 {
		...
		meta.StreamCompleted = done
	}

	p.Parser.UpdateMeta(meta)
	p.Parser.Close()
	if errParser := p.Parser.Err(); errParser != nil {
		return meta, p.wrapParserError(errParser)
	}
	return meta, nil
}

func (p *UpstreamPump) wrapParserError(err error) error {
	return &UpstreamPumpError{Phase: "parse", Cause: err, ModelOutput: p.Parser.RawText()}
}

func upstreamSSEError(data, eventType string) error {
	root := gjson.Parse(data)
	rootType := root.Get("type").String()
	if eventType != "error" && rootType != "error" && eventType != "response.failed" && rootType != "response.failed" {
		return nil
	}
	message := strings.TrimSpace(root.Get("error.message").String())
	if message == "" {
		message = strings.TrimSpace(root.Get("response.error.message").String())
	}
	if message == "" {
		message = strings.TrimSpace(root.Get("error.type").String())
	}
	if message == "" {
		message = strings.TrimSpace(root.Get("response.error.type").String())
	}
	if message == "" {
		message = "unknown upstream stream error"
	}
	return fmt.Errorf("upstream error event: %s", message)
}
...
func (e *UpstreamPumpError) Error() string {
	msg := fmt.Sprintf("toolemu upstream pump: %s: %v", e.Phase, e.Cause)
	if e.ModelOutput != "" {
		msg += "\nraw model output:\n" + e.ModelOutput
	}
	return msg

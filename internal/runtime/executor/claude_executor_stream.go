...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if helps.ToolEmuActive(ctx, e.Identifier(), baseModel, requestedModel, bodyForUpstream) {
		...
		if errHeaders := applyClaudeHeaders(httpReq, auth, apiKey, true, extraBetas, foldedForUpstream, e.cfg, incomingHeaders, confirmedClaudeCode && !cloaked, claudeSessionID); errHeaders != nil {
		...
		go func() {
			...
			upMeta, errStream := helps.RunToolEmuStream(ctx, meta, toolemu.ShapeClaudeMessages, decodedBody, choice, policy, func(frame []byte) {
				frame = restoreClaudeOAuthToolNamesFromStreamFrame(frame, func(line []byte) []byte {

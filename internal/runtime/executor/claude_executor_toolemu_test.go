...
func TestClaudeExecutor_ToolEmuNonStream(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Request-Id", "req_tool_emu")
	...
	if got := receivedAnthropicVersions[0]; got != "2026-07-19" {
		t.Fatalf("Anthropic-Version = %q, want caller version %q", got, "2026-07-19")
	}
	if got := resp.Headers.Get("Request-Id"); got != "req_tool_emu" {
		t.Fatalf("Request-Id = %q, want req_tool_emu", got)
	}
...
func TestClaudeExecutor_ToolEmuErrorBodyPreserved(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	for _, stream := range []bool{false, true} {
		t.Run(fmt.Sprintf("stream=%t", stream), func(t *testing.T) {
			req := cliproxyexecutor.Request{Model: "claude-test", Payload: payload}
			opts := cliproxyexecutor.Options{SourceFormat: sdktranslator.FromString("claude"), ResponseFormat: sdktranslator.FromString("claude"), OriginalRequest: payload}
			var err error
			if stream {
			...
			var rateLimitErr claudeRateLimitError
			if !errors.As(err, &rateLimitErr) || !rateLimitErr.IsCredentialScoped() {
				t.Fatalf("error = %v, want credential-scoped rate limit", err)
			}
			if delay := rateLimitErr.RetryAfter(); delay == nil || *delay < time.Minute {
				t.Fatalf("RetryAfter = %v, want at least one minute", delay)
			}
		})
	}
}

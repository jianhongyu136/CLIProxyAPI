...
func (e *ClaudeExecutor) buildClaudeToolEmuSend(auth *cliproxyauth.Auth, url, apiKey string, extraBetas []string, signCCH bool, entrypoint string, helperProfile bool, incomingHeaders http.Header, confirmedClaudeCode bool, claudeSessionID string, responseHeaders *http.Header) toolemu.UpstreamSendFunc {
	provider := e.upstreamRequestLogProvider()
	return func(ctx context.Context, body []byte) ([]byte, error) {
		bodyToSend := finalizeClaudeToolEmuFoldedBody(body)
		if signCCH {
			cchBilling := ""
			if !helperProfile || claudeBodyNeedsBillingFallback(bodyToSend) {
		...
		if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
			...
			return nil, classifyClaudeUpstreamError(httpResp.StatusCode, httpResp.Header, data)
		}

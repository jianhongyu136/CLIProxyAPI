...
func (e *OpenAICompatExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if helps.ToolEmuActive(ctx, e.Identifier(), baseModel, requestedModel, translated) {
		...
		send := e.buildToolEmuSend(ctx, auth, url, apiKey, opts.Headers)
...
func (e *OpenAICompatExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if helps.ToolEmuActive(ctx, e.Identifier(), baseModel, requestedModel, translated) {
		...
		tools, errTools := toolemu.ExtractToolSpecs(translated, toolemu.ShapeOpenAIChat)
		if errTools != nil {
			return nil, errTools
		}
		policy.Tools = tools
		folded, errFold := toolemu.FoldRequest(translated, toolemu.FoldOpts{Shape: toolemu.ShapeOpenAIChat, Provider: e.Identifier(), FenceToken: policy.FenceToken, TagGroup: policy.TagGroup})
		if errFold != nil {
			return nil, errFold
		}
		...
		if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
			...
			return nil, newOpenAICompatStatusError(httpResp.StatusCode, httpResp.Header, b)
		}
		...
		go func() {
			...
			_, errStream := helps.RunToolEmuStream(ctx, meta, toolemu.ShapeOpenAIChat, httpResp.Body, choice, policy, func(frame []byte) {
...
func (e *OpenAICompatExecutor) buildToolEmuSend(ctx context.Context, auth *cliproxyauth.Auth, url, apiKey string, incomingHeaders http.Header) toolemu.UpstreamSendFunc {
	provider := e.Identifier()
	return func(sctx context.Context, body []byte) ([]byte, error) {
		...
		util.ApplyCustomHeadersFromAttrs(httpReq, attrs, incomingHeaders)
		var authID, authLabel, authType, authValue string
		if auth != nil {
			authID = auth.ID
			authLabel = auth.Label
		...
		if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
			return nil, newOpenAICompatStatusError(httpResp.StatusCode, httpResp.Header, respBody)
		}
		return respBody, nil
	}
}

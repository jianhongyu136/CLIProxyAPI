...
func TestOpenAICompatExecutorToolEmuNonStream(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		forwardedHeader = r.Header.Get("X-Forwarded-Session")
	...
	resp, err := exec.Execute(context.Background(), &cliproxyauth.Auth{Attributes: map[string]string{"base_url": server.URL + "/v1", "api_key": "test", "header:X-Forwarded-Session": "$X-Client-Session"}}, cliproxyexecutor.Request{
		Model:   "gpt-test",
	...
	}, cliproxyexecutor.Options{SourceFormat: sdktranslator.FromString("openai"), Headers: http.Header{"X-Client-Session": []string{"tool-emu-session"}}})
	if err != nil {
		t.Fatalf("Execute error: %v", err)
	}
	if forwardedHeader != "tool-emu-session" {
		t.Fatalf("X-Forwarded-Session = %q, want tool-emu-session", forwardedHeader)
	}
...
func TestOpenAICompatExecutorToolEmuStream(t *testing.T) {
	...
	exec := NewOpenAICompatExecutor("openai-compatibility", &config.Config{ToolEmulation: toolemu.ToolEmulationConfig{Enabled: true}})
	result, err := exec.ExecuteStream(context.Background(), &cliproxyauth.Auth{Attributes: map[string]string{"base_url": server.URL + "/v1", "api_key": "test", "header:X-Forwarded-Session": "$X-Client-Session"}}, cliproxyexecutor.Request{
		Model:   "gpt-test",
	...
	}, cliproxyexecutor.Options{SourceFormat: sdktranslator.FromString("openai"), Stream: true, Headers: http.Header{"X-Client-Session": []string{"tool-emu-session"}}})
	if err != nil {
